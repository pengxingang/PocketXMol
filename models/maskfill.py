...
class AsymDiff(Module):
    def __init__(self,
        ...
        **kwargs
        ...
        # additional input
        self.add_features = config.add_features
        n_add = 0
        if 'fixed' in self.add_features:
            n_add += 1
        if 'info_level' in self.add_features:
            n_add += 1
        self.use_fixed_halfdist = ('fixed_halfdist' in self.add_features)
        
        self.nodetype_embedder = nn.Embedding(num_node_types, 
                                node_dim-2*(n_add))  # (2 for nodetype and pos)
        self.edgetype_embedder = nn.Embedding(num_edge_types,
                                              edge_dim-n_add-int(self.use_fixed_halfdist)) # bond type
        
        # # denoiser
        if config.denoiser.backbone == 'NodeEdgeNet':
            self.denoiser = NodeEdgeNet(node_dim, edge_dim, **config.denoiser)
        elif config.denoiser.backbone == 'NodeEdgeNetGVP':
            self.denoiser = NodeEdgeNetGVP(node_dim, edge_dim, **config.denoiser)
        else:
        ...
        self.node_decoder = MLP(node_dim, num_node_types, node_dim)
        self.edge_decoder = MLP(edge_dim, num_edge_types, edge_dim)
        # self.pos_corrector = PosCorrector()
        
        # additional output
        self.add_output = getattr(config, 'add_output', [])
        if 'error' in self.add_output:
            self.node_error = MLP(node_dim, 1, node_dim)
            self.pos_error = nn.Sequential(MLP(node_dim, 1, node_dim), nn.ReLU())
            self.edge_error = MLP(edge_dim, 1, edge_dim)
    ...
    def forward(self, batch, **kwargs):
        ...
        # # 1. prepare embedding 
        node_in, pos_in, halfedge_in = batch['node_in'], batch['pos_in'], batch['halfedge_in']
        h_node_in = self.nodetype_embedder(node_in)
        h_halfedge_in = self.edgetype_embedder(halfedge_in)
        
        if 'fixed' in self.add_features:
            fixed_node, fixed_pos, fixed_halfedge = batch['fixed_node'], batch['fixed_pos'], batch['fixed_halfedge']
            fixed_node = fixed_node.unsqueeze(-1).to(pos_in.dtype)
            fixed_pos = fixed_pos.unsqueeze(-1).to(pos_in.dtype)
            fixed_halfedge = fixed_halfedge.unsqueeze(-1).to(pos_in.dtype)
            h_node_in = torch.cat([
                h_node_in, fixed_node, fixed_pos,
            ], dim=-1)
            h_halfedge_in = torch.cat([
                h_halfedge_in, fixed_halfedge,
            ], dim=-1)
            if self.use_fixed_halfdist:
                fixed_halfdist = batch['fixed_halfdist'].unsqueeze(-1).to(pos_in.dtype)
                h_halfedge_in = torch.cat([
                    h_halfedge_in, fixed_halfdist,
                ], dim=-1)
        if 'info_level' in self.add_features: #TODO: level need embedding?
            level_node, level_pos, level_halfedge = batch['level_node'], batch['level_pos'], batch['level_halfedge']
            h_node_in = torch.cat([
                h_node_in,
                level_node.unsqueeze(-1).to(pos_in.dtype),
                level_pos.unsqueeze(-1).to(pos_in.dtype),
            ], dim=-1)
            h_halfedge_in = torch.cat([
                h_halfedge_in,
                level_halfedge.unsqueeze(-1).to(pos_in.dtype),
            ], dim=-1)
        

        # break symmetry
        n_halfedges = halfedge_in.shape[0]
        halfedge_index = batch['halfedge_index']
        edge_index = torch.cat([halfedge_index, halfedge_index.flip(0)], dim=1)
        h_edge_in = torch.cat([h_halfedge_in, h_halfedge_in], dim=0)
        fixed_edge = torch.cat([fixed_halfedge, fixed_halfedge], dim=0)
        ...
        h_node, pos_node, h_edge = self.denoiser(
            ...
            h_edge=h_edge_in, 
            edge_index=edge_index,
            node_time=fixed_node,
            edge_time=fixed_edge,
        )
        ...
        additional_outputs = {}
        if 'error' in self.add_output:
            pred_node_error = self.node_error(h_node)
            pred_pos_error = self.pos_error(h_node)  # use the node hidden
            pred_edge_error = self.edge_error(h_edge[:n_halfedges]+h_edge[n_halfedges:])  # NOTE why not divide by 2?
            additional_outputs = {'pred_node_error': pred_node_error, 'pred_pos_error': pred_pos_error, 'pred_halfedge_error': pred_edge_error}
        elif 'dihedral' in self.add_output:
            sin_out, cos_out = get_dihedral_batch(pred_pos, batch['tor_bonds_anno'], batch['dihedral_pairs_anno'])
            additional_outputs.update({'dih_sin': sin_out, 'dih_cos': cos_out})
